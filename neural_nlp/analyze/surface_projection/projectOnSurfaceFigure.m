function projectOnSurfaceFigure(fileNames, config)
...
for f = 1:nFiles
    dotInds = strfind(fileNames{f}, '.');    
    currFormat = fileNames{f}((dotInds(end)+1):end);
...
fileSizes = nan(nFiles+1,3);
for f = 1:nFiles
    volInfo = spm_vol(fileNames{f});
    fileSizes(f,:) = volInfo.dim;
end
if config.doCoreg
    volInfo = spm_vol(subj_T1_path);    
    fileNames = [fileNames; subj_T1_path];
else
    volInfo = spm_vol(FS_T1_path);
    fileNames = [fileNames; FS_T1_path];    
end
fileSizes(end,:) = volInfo.dim;
...
for f = 1:numel(badInds)
    clear imcalcInfo    
    if isempty(find(badInds == nFiles+1, 1))
    ...
    imcalcInfo.interp = config.interp;             % nearest neighbor
    runImcalc_v2018(imcalcInfo); 
    fileNames{badInds(f)} = imcalcInfo.output;
end    
subj_T1_path = fileNames{end};
fileNames = fileNames(1:end-1);

%% Coregister all files to SPM's T1 %%
if config.doCoreg
    load Coreg_SPM12
    matlabbatch{1}.spm.spatial.coreg.estwrite.ref = {SPM_T1_path};
    matlabbatch{1}.spm.spatial.coreg.estwrite.source = {subj_T1_path};
...
if config.doCoreg
    load Coreg_SPM12
    matlabbatch{1}.spm.spatial.coreg.estwrite.ref = {FS_T1_path};
    matlabbatch{1}.spm.spatial.coreg.estwrite.source = {SPM_T1_path};
    matlabbatch{1}.spm.spatial.coreg.estwrite.other = fileNames;
    matlabbatch{1}.spm.spatial.coreg.estwrite.roptions.interp = config.interp;
