function transformToSurface(fileNames, config, freeview)
...
%
% Idan Blank, Aug 05 2019, edits: Greta Tuckute, February 2020
...
% New freesurfer directories on openmind
setupCommands = ['export FREESURFER_HOME=/cm/shared/openmind/freesurfer/6.0.0; ', ...
    'export SUBJECTS_DIR=/cm/shared/openmind/freesurfer/6.0.0/subjects; ', ...
    'source $FREESURFER_HOME/SetUpFreeSurfer.sh; '];

freeviewSurfaceDir = '/cm/shared/openmind/freesurfer/6.0.0/subjects/cvs_avg35_inMNI152/surf/';
...
for f = 1:nFiles
    ...
    % If the current format is NOT .nii
...
for f = 1:nFiles
    volInfo = spm_vol(fileNames{f}); % get the volume info from the .nii brainmatrix file
    fileSizes(f,:) = volInfo.dim; % adds the file size as the first row
end
if config.doCoreg 
    volInfo = spm_vol(subj_T1_path); % 
    fileNames = [fileNames; subj_T1_path]; % now filenames contain: 1st row: brainmatrix.nii, 2nd row: the subject reg file.nii
else
    volInfo = spm_vol(FS_T1_path);
    fileNames = [fileNames; FS_T1_path];    
end
fileSizes(end,:) = volInfo.dim; % adds the FS subject reg file dimension in 2nd row
...
disp(['Making sure all files have the following dimensions: ', mat2str(modeRow)]); % making sure files have the FS reg file dimension

for f = 1:numel(badInds) 
    clear imcalcInfo    
    if isempty(find(badInds == nFiles+1, 1)) % badInds = 1
        imcalcInfo.input{1} = fileNames{end}; 
    else
        imcalcInfo.input{1} = fileNames{modeRowInd};        
    end
    imcalcInfo.input{2} = fileNames{badInds(f)}; 
    ...
    imcalcInfo.interp = config.interp;             
...
if config.doCoreg
    ...
    matlabbatch{1}.spm.spatial.coreg.estwrite.other = fileNames; % the brainmatrix.nii file
...
if config.doCoreg
    ...
    matlabbatch{1}.spm.spatial.coreg.estwrite.source = {SPM_T1_path}; % source SPM T1 file
    matlabbatch{1}.spm.spatial.coreg.estwrite.other = fileNames; 
    matlabbatch{1}.spm.spatial.coreg.estwrite.roptions.interp = config.interp;
    matlabbatch{1}.spm.spatial.coreg.estwrite.eoptions.fwhm = [0, 0]  
...
for f = 1:nFiles
    ...
    currCommand = ['mri_vol2surf --mov ', fileNames{f}, ' --o ', surfaceFileName, ...
        ' --hemi ', config.hemi, ' --fwhm 0', ' --regheader cvs_avg35_inMNI152', ... 
        ' --', config.projectStr]; 
    [status] = system([setupCommands, currCommand], '-echo')
    
    if freeview
        figName = [origFileNames{f}(1:(end-4)), '_', config.hemi, '_az', num2str(config.azimuth), '.png'];
